...
WORD cpu_getr_w(BYTE reg) {
	if (reg > 0x3C) {
		if (reg > 0xF0) return CPU_STATE.REG.word[reg & 0xF];
		if (reg > 0xE0) return CPU_STATE.BANK.word[((CPU_STATE.RFP + 7) % 8) + (reg & 0xF)];
...
DWORD cpu_getr_dw(BYTE reg) {
	if (reg > 0x3C) {
		if (reg > 0xF0) return CPU_STATE.REG.dword[reg & 0xF];
		if (reg > 0xE0) return CPU_STATE.BANK.dword[((CPU_STATE.RFP + 7) % 8) + (reg & 0xF)];
...
void cpu_setr_w(BYTE reg, WORD value) {
	if (reg > 0x3C) {
		if (reg > 0xF0) CPU_STATE.REG.word[reg & 0xF] = value;
		else if (reg > 0xE0) CPU_STATE.BANK.word[((CPU_STATE.RFP + 7) % 8) + (reg & 0xF)] = value;
...
void cpu_setr_dw(BYTE reg, DWORD value) {
	if (reg > 0x3C) {
		if (reg > 0xF0) CPU_STATE.REG.dword[reg & 0xF] = value;
		else if (reg > 0xE0) CPU_STATE.BANK.dword[((CPU_STATE.RFP + 7) % 8) + (reg & 0xF)] = value;
...
WORD cpu_getR_w(BYTE reg) {
	return CPU_STATE.BANK.word[CPU_STATE.RFP + reg_R[reg]];
}

DWORD cpu_getR_dw(BYTE reg) {
	return CPU_STATE.BANK.dword[CPU_STATE.RFP + reg_R[reg]];
}
...
void cpu_setR_w(BYTE reg, WORD value) {
	CPU_STATE.BANK.word[CPU_STATE.RFP + reg_R[reg]] = value;
}

void cpu_setR_dw(BYTE reg, DWORD value) {
	CPU_STATE.BANK.dword[CPU_STATE.RFP + reg_R[reg]] = value;
}